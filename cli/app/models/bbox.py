...
class BBoxDim(BBoxNorm):
  ...
  def to_expanded(self, ltrb, redistribute=True):
    ...
    # expand
    # l, t, r, b = ltrb
    # r = list( (np.array(self.xyxy) + np.array([-l, -t, r, b])).astype('int'))
    # # outliers
    # oob = list(range(4))
    # oob[0] = min(r[0], 0)
    # oob[1] = min(r[1], 0)
    # oob[2] = self.dim[0] - r[2]
    # oob[3] = self.dim[1] - r[3]
    # oob = np.array(oob)
    # oob[oob > 0] = 0
    # # absolute amount
    # oob = np.absolute(oob)
    ...
    # r[0] = max(r[0], 0)
    # r[1] = max(r[1], 0)
    # r[2] = min(r[2], self.dim[0])
    # r[3] = min(r[3], self.dim[1])
    ...
    # if np.array(oob).any():
    #   m = np.max(oob)
    #   # is this each index or only max amount?
